...
if (!install_local) remotes::install_github("pbs-assess/sdmTMB", dependencies = TRUE, ref="newlogistic", force=T)
library(sdmTMB)
...
#Randomly assign folds (with specified seed)
seed <- 42 #872 #1929
set.seed(seed)
k_folds <- 10
dat$fold_ids <- sample(seq_len(k_folds), nrow(dat), replace = TRUE)
#Leave out only 10% of data
test_10 <- T
if(test_10){
  k_folds <- 2
 dat$fold_idsclust <- sample(1:2, size = nrow(dat), replace = T, prob = c(0.1, 0.9))
}
future::plan(future::multisession)
start <- matrix(c(20, 1.1), ncol = 1L) #sablefish
...
start <- matrix(c(1.4, 0.1, 50, 0.1), ncol=1)
...
start <- init_vals$"Sebastolobus altivelis"$m3$start
start <- matrix(c(-0.9, -0.6,9))
...
### Create log(prediction) (ie loss) table ###
...
## Create table and add loss for each (totals) ##
compare<- as.data.frame(matrix(NA, ncol = 1, nrow =length(models), dimnames = list(models)))
compare[1,] <- m1$sum_loglik
compare[2,] <- m2$sum_loglik
compare[3,] <- m2a$sum_loglik
compare[4,] <- m3$sum_loglik
compare[5,] <- m4$sum_loglik
compare[6,] <- m5$sum_loglik
compare[7,] <- m6$sum_loglik
compare[8,] <- m7$sum_loglik
compare[9,] <- m8$sum_loglik

compare[1,] <- m1$fold_loglik[1]
compare[2,] <- m2$fold_loglik[1]
compare[3,] <- m2a$fold_loglik[1]
compare[4,] <- m3$fold_loglik[1]
compare[5,] <- m4$fold_loglik[1]
compare[6,] <- m5$fold_loglik[1]
compare[7,] <- m6$fold_loglik[1]
compare[8,] <- m7$fold_loglik[1]
compare[9,] <- m8$fold_loglik[1]

compare[1,2] <- m1$fold_loglik[1]
compare[2,2] <- m2$fold_loglik[1]
compare[3,2] <- m2a$fold_loglik[1]
compare[4,2] <- m3$fold_loglik[1]
compare[5,2] <- m4$fold_loglik[1]
compare[6,2] <- m5$fold_loglik[1]
compare[7,2] <- m6$fold_loglik[1]
compare[8,2] <- m7$fold_loglik[1]
compare[9,2] <- m8$fold_loglik[1]

compare$dLL <- abs(max(compare$V1)-(compare$V1))

## For each fold ##

save <- F
if(save){
save(m1,m2,m2a,m3,m4,m5,m6,m7,m8, file="models_thornyhead.R")
save(compare, compare2, file="thornyhead_model_selection.R")
}
