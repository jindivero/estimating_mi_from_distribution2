...
compare2 <- lapply(simdat2, run_alt_models, start, mesh)
...
AIC1$model <- row.names(AIC1)
AIC1$model <- str_sub(AIC1$model, end=-4)

AIC2 <- keep(.x=compare2, .p=is.data.frame)
AIC2 <- bind_rows(compare2, .id="df")
AIC2$model <- row.names(AIC2)
AIC2$model <- str_sub(AIC2$model, end=-4)
