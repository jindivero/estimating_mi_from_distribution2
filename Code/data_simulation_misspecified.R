...
smax <- 10 # maximum effect of MI; had been 4 in previous data simulation
Eo <- 0.3
Eo2 <- 0.7
...
simulate_fish<- function(dat,mesh, x50, delta, smax, Eo) {
  ...
  beta_depth <- c(0.8091755, -0.9387189,  1.0734900, -0.1599094, -0.2128697) # fit 5th order polynomial to smoother on sablefish
  ...
  dat$log_depth_scaled3 <- dat$log_depth_scaled^3
  dat$log_depth_scaled4 <- dat$log_depth_scaled^4
  dat$log_depth_scaled5 <- dat$log_depth_scaled^5
  
  sim <- sdmTMB_simulate(formula=~-1+as.factor(year)+logistic(mi)+
                           log_depth_scaled + log_depth_scaled2 + log_depth_scaled3 + log_depth_scaled4 + log_depth_scaled5,
                         data=dat,
...
ggplot(dat, aes(x=po2_s,y=cpue_kg_km2))+geom_point()+geom_point(sim_test, mapping=aes(x=po2_s, y=sim), color="blue")
ggplot(sim_test, aes(x=po2,y=sim))+geom_point()
ggplot(sim_test2, aes(x=po2,y=sim))+geom_point()
...
save(fits, fits2, fits3, fits4, file="model_fits_smoothdepth.Rdata")
}
